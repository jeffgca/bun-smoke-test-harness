...
description: 'Working directory used for Bun installations and test checkout',
})
.option('testsRepo', {
type: 'string',
...
const testsDir = resolve(argv.testsDir ?? join(workspace, 'bun-functional-tests'))
const reportFile = resolve(argv.reportFile ?? join(workspace, 'comparison-report.json'))
...
const canaryBinary = join(
bunInstallDir,
'bin',
process.platform === 'win32' ? 'bun.exe' : 'bun',
)

if (!existsSync(canaryBinary)) {
throw new Error(`Canary Bun binary not found at ${canaryBinary}`)
}
...
const pullResult = runCommand(['git', '-C', checkoutDir, 'pull', '--ff-only'], workspace)
ensureSuccess('update bun-functional-tests', pullResult)
return
}

mkdirSync(dirname(checkoutDir), { recursive: true })
const cloneResult = runCommand(['git', 'clone', repoUrl, checkoutDir], workspace)
